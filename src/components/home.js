    ...
    <div id="home">
      <div className="main-navbar">
        <div>
          <Link to="/">
            <h1 className="title">
              <i className="fab fa-instalod"></i> SJoy
            </h1>
          </Link>
        </div>
        <ul>
          <li onClick={getInfo}>
            <i className="title fas fa-user-secret"></i>
          </li>
        </ul>
        <div id="user-info">
          <img
      ...
      <div className="body-section">
        <div className="history">
          ...
          {dmusers != null && dmusers.length > 0 && !loading ? (
            dmusers.map((uid, index) => {
              return <User key={index} userid={uid} loginid={id} />;
            })
          ) : dmusers != null && dmusers.length == 0 ? (
            <h1 className="secondary" style={{ textAlign: "center" }}>
          ...
          <div className="form-section">
            ...
            <div className="result">
              {matchusers.length > 0 && !loading ? (
                ...
                <h1 className="secondary" style={{ textAlign: "center" }}>
                  <i className="fas fa-poll"></i>Search Results appear here
                </h1>
              )}
            </div>
          </div>
